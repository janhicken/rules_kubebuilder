...
def _kubebuilder_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "KUBEBUILDER_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    kubebuilderinfo = KubebuilderInfo(
...
kubebuilder_toolchain = rule(
    implementation = _kubebuilder_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a kubebuilder compiler/runtime toolchain.
